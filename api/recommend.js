export default async function handler(req, res) {
  ...
  const { prompt, raw, worksheetMode, writingFrameMode, comprehensionMode, exitTicketMode, vocabCardsMode, knowledgeOrgMode, bingoMode, wordsearchMode, slideshowMode, assistantMode, history } = req.body
  if (!prompt) return res.status(400).json({ error: 'Missing prompt' })

  // Server-side input length limit to prevent abuse
  ...
  if (assistantMode) {
    try {
      ...
      const body = {
        ...
        max_tokens: 2000,
        system: 'You are a helpful UK primary school teaching assistant. Help teachers with lesson planning, differentiation, SEND support, assessment, behaviour management, and curriculum advice. Keep responses practical and relevant to UK primary education.',
        messages,
      }
