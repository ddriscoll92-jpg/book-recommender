...
const s = {
  ...
  resultsHeader: { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: `0.5px solid ${BORDER}`, flexWrap: 'wrap' },
  resultsTitle: { fontFamily: "'Lora', serif", fontSize: 16, fontWeight: 500, color: MUTED, flex: '1 1 200px', minWidth: 0, margin: 0 },
  badge: { background: LIGHT_GREEN, color: '#085041', fontSize: 11, fontWeight: 500, padding: '3px 10px', borderRadius: 20, whiteSpace: 'nowrap', flexShrink: 0 },
  ...
  subjectTile: (active) => ({ padding: '14px 10px', borderRadius: 10, border: `0.5px solid ${active ? GREEN : BORDER}`, background: active ? LIGHT_GREEN : BG, cursor: 'pointer', textAlign: 'center', transition: 'all 0.15s', minHeight: 84, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }),
  subjectEmoji: { fontSize: 22, display: 'block', marginBottom: 4, lineHeight: 1, height: 26 },
  ...
  proceedBar: { position: 'sticky', bottom: 16, background: BG, border: `0.5px solid ${BORDER}`, borderRadius: 12, padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', marginTop: 8, gap: 10, flexWrap: 'wrap' },
  proceedBtn: (disabled) => ({ height: 40, padding: '0 20px', background: disabled ? '#888780' : GREEN, color: LIGHT_GREEN, border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: disabled ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: "'DM Sans', sans-serif", whiteSpace: 'nowrap', flexShrink: 0 }),
}
...
async function downloadSlideshowPptx(slideshow, lesson, plan, group) {
  ...
  const NAVY_HEX = '1E2433'
  const GREEN_HEX = '1D9E75'
  const LIGHT_GREEN_HEX = 'E1F5EE'
  const MUTED_HEX = '5F5E5A'
  const TEXT_HEX = '2C2C2A'
  const PAGE_BG_HEX = 'F5F4F0'
  const TIER_COLOURS = {
    support: 'DC2626', supportBg: 'FEF2F2',
    core: 'D97706', coreBg: 'FEF3C7',
    extension: '16A34A', extensionBg: 'F0FDF4',
  }
  ...
  ;(slideshow.slides || []).forEach((sl, idx) => {
    const slide = pptx.addSlide()
    const pageLabel = `${idx + 1} / ${slideshow.slides.length}`

    if (sl.type === 'title') {
      slide.background = { color: NAVY_HEX }
      slide.addText(sl.heading || slideshow.title || '', { x: margin, y: 1.8, w: W - margin*2, h: 1.2, fontSize: 32, bold: true, color: 'FFFFFF', fontFace: 'Cambria', align: 'left' })
      slide.addText(sl.subheading || `${group?.yearGroup || ''} · ${plan?.subject || ''} · ${group?.book?.title || ''}`, { x: margin, y: 3.0, w: W - margin*2, h: 0.5, fontSize: 14, color: '8B93A7', fontFace: 'Calibri' })
      slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x: margin, y: 1.4, w: 0.6, h: 0.6, fill: { color: GREEN_HEX }, rectRadius: 0.1, line: { type: 'none' } })
      slide.addText('🎬', { x: margin, y: 1.4, w: 0.6, h: 0.6, fontSize: 22, align: 'center', valign: 'middle' })
      addFooter(slide, pageLabel)
      return
    }

    // Default white background with NAVY header bar for all other slide types
    slide.background = { color: 'FFFFFF' }
    slide.addShape(pptx.shapes.RECTANGLE, { x: 0, y: 0, w: W, h: 0.9, fill: { color: NAVY_HEX }, line: { type: 'none' } })
    slide.addText(sl.heading || '', { x: margin, y: 0.15, w: W - margin*2, h: 0.6, fontSize: 22, bold: true, color: 'FFFFFF', fontFace: 'Cambria', valign: 'middle' })

    const bodyTop = 1.15
    const bodyH = H - bodyTop - 0.4

    if (sl.type === 'objective') {
      slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x: margin, y: bodyTop + 0.4, w: W - margin*2, h: 1.4, fill: { color: LIGHT_GREEN_HEX }, rectRadius: 0.08, line: { type: 'none' } })
      slide.addText(sl.content || '', { x: margin + 0.3, y: bodyTop + 0.4, w: W - margin*2 - 0.6, h: 1.4, fontSize: 18, color: '085041', bold: true, fontFace: 'Calibri', valign: 'middle', align: 'left' })
    } else if (sl.type === 'vocab') {
      const items = sl.items || []
      const cols = 2
      const cardW = (W - margin*2 - 0.3) / cols
      const cardH = 1.0
      items.slice(0, 6).forEach((item, i) => {
        const col = i % cols
        const row = Math.floor(i / cols)
        const x = margin + col * (cardW + 0.3)
        const y = bodyTop + row * (cardH + 0.2)
        slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x, y, w: cardW, h: cardH, fill: { color: PAGE_BG_HEX }, rectRadius: 0.06, line: { color: 'D3D1C7', width: 0.75 } })
        slide.addText(item.word || '', { x: x + 0.15, y: y + 0.08, w: cardW - 0.3, h: 0.35, fontSize: 14, bold: true, color: GREEN_HEX, fontFace: 'Calibri' })
        slide.addText(item.definition || '', { x: x + 0.15, y: y + 0.42, w: cardW - 0.3, h: cardH - 0.5, fontSize: 11, color: MUTED_HEX, fontFace: 'Calibri', valign: 'top' })
      })
    } else if (sl.type === 'differentiation') {
      const cols = [
        { key: 'support', label: 'Support' },
        { key: 'core', label: 'Core' },
        { key: 'extension', label: 'Extension' },
      ]
      const colW = (W - margin*2 - 0.4) / 3
      cols.forEach((c, i) => {
        const x = margin + i * (colW + 0.2)
        const accent = TIER_COLOURS[c.key]
        const accentBg = TIER_COLOURS[c.key + 'Bg']
        slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x, y: bodyTop, w: colW, h: 0.5, fill: { color: accent }, rectRadius: 0.06, line: { type: 'none' } })
        slide.addText(c.label, { x, y: bodyTop, w: colW, h: 0.5, fontSize: 13, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', fontFace: 'Calibri' })
        slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x, y: bodyTop + 0.55, w: colW, h: bodyH - 0.55, fill: { color: accentBg }, rectRadius: 0.06, line: { type: 'none' } })
        slide.addText(sl[c.key] || '', { x: x + 0.12, y: bodyTop + 0.65, w: colW - 0.24, h: bodyH - 0.75, fontSize: 11, color: TEXT_HEX, fontFace: 'Calibri', valign: 'top' })
      })
    } else if (sl.bullets && sl.bullets.length) {
      let textY = bodyTop + 0.3
      if (sl.content) {
        slide.addText(sl.content, { x: margin, y: textY, w: W - margin*2, h: 0.6, fontSize: 14, color: TEXT_HEX, fontFace: 'Calibri', italic: true })
        textY += 0.7
      }
      const bullets = sl.bullets.map((b, bi) => ({ text: b, options: { bullet: true, color: TEXT_HEX, fontSize: 15, breakLine: bi < sl.bullets.length - 1, paraSpaceAfter: 10 } }))
      slide.addText(bullets, { x: margin + 0.2, y: textY, w: W - margin*2 - 0.4, h: bodyH - (textY - bodyTop), fontFace: 'Calibri', valign: 'top' })
    } else if (sl.content) {
      slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x: margin, y: bodyTop + 0.3, w: W - margin*2, h: bodyH - 0.5, fill: { color: PAGE_BG_HEX }, rectRadius: 0.08, line: { type: 'none' } })
      slide.addText(sl.content, { x: margin + 0.3, y: bodyTop + 0.3, w: W - margin*2 - 0.6, h: bodyH - 0.5, fontSize: 16, color: TEXT_HEX, fontFace: 'Calibri', valign: 'middle' })
    }

    addFooter(slide, pageLabel)
  })

  await pptx.writeFile({ fileName: `${(slideshow.title || lesson?.title || 'lesson').replace(/[^a-z0-9]/gi,'_')}_slideshow.pptx` })
}
    ...
    for (let row = 0; row < 2; row++) {
      for (let col = 0; col < 2; col++) {
        ...
        const measured = tier.prompts.map(p => {
          ...
          const pLines = doc.splitTextToSize(p.text, lineW).slice(0,4)
          return { p, pLines, textH: pLines.length * 3.8 }
        })
...
async function downloadWorksheetPdf(worksheet) {
  ...
  worksheet.tiers.forEach((tier, ti) => {
    ...
    tier.questions.forEach((q, qi) => {
      ...
      doc.roundedRect(qx, qy, colW, rowH - 2, 1.5, 1.5, 'FD')
...
function ResourceOutput({ resource }) {
  ...
  async function handleDownload(format) {
    ...
    try {
      if (format === 'txt') {
        const lines = []
        lines.push(resourceToUse.title)
        lines.push('='.repeat(60))
        lines.push(resourceToUse.meta || '')
        lines.push('')
        ...
        const a = document.createElement('a')
        a.href = url
        a.download = `${resourceToUse.title.replace(/[^a-z0-9]/gi, '_')}.txt`
        a.click()
        ...
        function sanitise(str) {
          return String(str || '')
            ...
            .replace(/^[\s%|+\-=*#~]{3,}.*$/gm, '')  // strip ASCII art border lines
        ...
        resourceToUse.sections?.forEach(sec => {
          ...
          doc.text(sanitise(sec.heading).toUpperCase(), margin + 4, y + 0.5); y += 9
          body(sec.content); y += 4
        })
        doc.save(`${resourceToUse.title.replace(/[^a-z0-9]/gi, '_')}.pdf`)
      } else if (format === 'docx') {
        ...
        const children = [
          new Paragraph({ children: [new TextRun({ text: resourceToUse.title, bold: true, color: '1E2433', size: 36 })], spacing: { after: 80 } }),
          new Paragraph({ children: [new TextRun({ text: resourceToUse.meta || '', color: '5F5E5A', size: 18, italics: true })], spacing: { after: 400 } }),
          ...(resourceToUse.sections?.flatMap(sec => [
        ...
        ;(window.saveAs || (window.FileSaver && window.FileSaver.saveAs))(blob, `${resourceToUse.title.replace(/[^a-z0-9]/gi, '_')}.docx`)
      }
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
              {activeTab === 'model' ? (
                ...
                async function handleCreateSlideshow() {
                  ...
                  setSlideshowError('')
                  try {
                    const slidePrompt = `Create a teaching slideshow for this lesson.
Lesson title: ${lesson.title}
Year group: ${group.yearGroup}
Subject: ${plan.subject}
Book: "${group.book.title}" by ${group.book.author}
Lesson overview: ${lesson.lesson_overview || ''}
Learning intention: ${lesson.learning_intention || ''}
Success criteria: ${(lesson.success_criteria || []).join('; ')}
Main activity: ${lesson.main_activity || ''}
...
SEND adaptations - Extension: ${(lesson.send_adaptations?.higher || []).join('; ')}`
                    const res = await fetch('/api/recommend', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ prompt: slidePrompt, slideshowMode: true }),
                    })
                    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || `API error ${res.status}`) }
                    const data = await res.json()
                    await downloadSlideshowPptx(data.slideshow, lesson, plan, group)
                  } catch(err) {
                    setSlideshowError(err.message || 'Something went wrong. Please try again.')
                  }
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                        ...
                        <button onClick={handleCreateSlideshow} disabled={generatingSlideshow}
                          style={{ height: 30, padding: '0 12px', background: generatingSlideshow ? PAGE_BG : GREEN, color: generatingSlideshow ? MUTED : LIGHT_GREEN, border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: generatingSlideshow ? 'wait' : 'pointer', fontFamily: "'DM Sans', sans-serif", display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
                          {generatingSlideshow ? '⏳ Generating...' : '🎬 Create slideshow'}
                        </button>
                      </div>
                    </div>
                    {slideshowError && (
                      <div style={{ background: '#FCEBEB', border: '0.5px solid #A32D2D', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#A32D2D', marginBottom: 12 }}>{slideshowError}</div>
                    )}
...
function PlansModal({ book, plans, onClose, onAddPlan, onViewPlan, onEditPlan, onDeletePlan, onViewUnit }) {
                ...
                {isEditing ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap' }}>
                    ...
                    <div style={{ display: 'flex', gap: 5, flexShrink: 0, flexWrap: 'wrap' }}>
                      ...
                      {onViewUnit && <button onClick={() => onViewUnit(plan.id)} title="Unit overview" style={{ height: 26, padding: '0 10px', background: BG, border: `0.5px solid ${BORDER}`, borderRadius: 6, fontSize: 11, fontWeight: 500, color: TEXT, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", whiteSpace: 'nowrap' }}>📖 Unit</button>}
...
function BookGridCard({ book, isFavourite, onToggleFavourite, onViewBook, onViewPlans, onCreatePlan, onEdit, onDelete }) {
  ...
  return (
    <div style={{ background: BG, border: `0.5px solid ${hovered ? GREEN : BORDER}`, borderRadius: 12, overflow: 'hidden', transition: 'all 0.15s', boxShadow: hovered ? '0 4px 16px rgba(0,0,0,0.08)' : 'none', display: 'flex', flexDirection: 'column' }}
      ...
      <div style={{ background: LIGHT_GREEN, height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        <span style={{ fontSize: 36, lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', height: 44, width: 44 }}>{subjectMeta?.emoji || '📚'}</span>
        {/* Star toggle */}
  ...
  return (
    ...
    {modal?.mode === 'plans' && (
      <PlansModal
        ...
        onViewUnit={planId => onNavigate('unit', planId)}
      />
    )}
...
CRITICAL FORMATTING RULES — the content will be rendered as a PDF using a basic font:
- Use plain text only. No emoji, no special characters, no symbols outside standard ASCII.
- Do NOT use ASCII art, box-drawing characters, or borders made of %, |, +, -, = or similar.
- Do NOT use bullet characters like •, *, –. Use plain numbered lists (1. 2. 3.) or dashes (-).
- Separate items with line breaks only.
- For vocab cards: list each term as "Term: [word] - Definition: [meaning]" on its own line.
- For knowledge organisers: include Key Facts, Key People/Vocabulary, and Key Dates/Events sections.
- For all other types: use plain prose or numbered lists only.`

    try {
      const result = await callAPI(apiPrompt, true)
      setResource({ ...result, _genId: Date.now() })
      try {
        const { data: { user } } = await supabase.auth.getUser()
        if (user) {
          const { error: _saveErr } = await supabase.from('resources').insert({
...
function PresentationDownloadButton({ presentation }) {
  const [downloading, setDownloading] = useState(false)
  async function handle() {
    setDownloading(true)
    try {
      await downloadSlideshowPptx({ title: presentation.title, slides: presentation.slides }, null, null, null)
    } catch(e) { console.error('Presentation download error:', e) }
    setDownloading(false)
  }
  return (
...
function PresentationsPage({ onNavigate, checkTrial, initialPlanId }) {
  ...
  async function loadCatalogue() {
    ...
    const { data, error } = await supabase
      ...
      .order('created_at', { ascending: false })
    if (!error) setCatalogue(data || [])
    else console.error('loadCatalogue (presentations) error:', error)
    setCatalogueLoading(false)
  }
  ...
  const [selectedPlanGroup, setSelectedPlanGroup] = useState(null)
  const [selectedPlan, setSelectedPlan] = useState(null)
  const [selectedLessonNums, setSelectedLessonNums] = useState([])
  const [planSearch, setPlanSearch] = useState('')
  const [planFilterSubject, setPlanFilterSubject] = useState('All')
  const [planFilterYear, setPlanFilterYear] = useState('All')
  const [planDropdownOpen, setPlanDropdownOpen] = useState(false)
  ...
  async function generateAndSave(slidePrompt, titleFallback, planId, fallbackSubject, fallbackYearGroup) {
    ...
    await downloadSlideshowPptx(slideshow, null, null, null)
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (user) {
        const { error: _saveErr } = await supabase.from('presentations').insert({
          user_id: user.id,
          plan_id: planId || null,
        ...
        if (_saveErr) console.error('Presentation save error:', _saveErr.message)
      }
    } catch(e) { console.error('Presentation save error:', e?.message) }
  }

  async function generateFromPlan() {
    if (!selectedPlan || selectedLessonNums.length === 0) return
    const allowed = await checkTrial?.('resources')
    if (allowed === false) return
    setGenerating(true); setError(''); setGeneratedCount(0)

    const lessonsToGenerate = realLessons.filter(l => selectedLessonNums.includes(l.lesson_number))
...
SEND adaptations - Extension: ${(lesson.send_adaptations?.higher || []).join('; ')}`
  ...
  async function generateAdhoc() {
    ...
    if (allowed === false) return
    setGenerating(true); setError(''); setGeneratedCount(0)
    const slidePrompt = `Create a teaching slideshow based on this request from a UK primary school teacher: ${prompt}`
    try {
      await generateAndSave(slidePrompt, 'Presentation', null)
      setGeneratedCount(1)
    } catch(err) {
  ...
  async function handleRegenerate(presentation) {
    setRegeneratingId(presentation.id)
    try {
      const slidePrompt = presentation.prompt || `Create a teaching slideshow titled "${presentation.title}"`
      const res = await fetch('/api/recommend', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: slidePrompt, slideshowMode: true }),
  ...
  const EXAMPLE_PROMPTS = [
    "Create a slideshow introducing fractions for Year 4",
  ...
  const tabBtn = (id, label, emoji) => (
    <button onClick={() => { setTab(id); setError(''); setGeneratedCount(0) }}
      style={{ flex: 1, height: 42, border: "none", borderBottom: tab === id ? `2px solid ${GREEN}` : "2px solid transparent", background: "transparent", cursor: "pointer", fontSize: 14, fontWeight: tab === id ? 600 : 400, color: tab === id ? GREEN : MUTED, fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
      {emoji} {label}
    </button>
  )

  // Plan tab computed values
  const allPlans = realPlans.flatMap(group => group.plans.map(plan => ({ ...plan, group })))
  const planSubjects = ['All', ...Array.from(new Set(allPlans.map(p => p.subject))).sort()]
  const planYears = ['All', ...Array.from(new Set(allPlans.map(p => p.group.yearGroup))).sort()]
  const filteredPlans = allPlans.filter(p => {
        ...
        <div style={{ background: BG, border: `0.5px solid ${BORDER}`, borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
          ...
          <div style={{ padding: "20px" }}>
            ...
            {tab === 'adhoc' && (
              <div>
                <p style={{ fontSize: 13, color: MUTED, marginBottom: 14, lineHeight: 1.6 }}>
                  Describe the lesson or topic you'd like a slideshow for. The AI will build a complete slide deck with a title slide, objective, teaching points, activity and plenary.
                </p>
                <textarea
                  style={{ ...s.textarea, height: 100, borderRadius: 8, borderBottom: `0.5px solid ${BORDER}`, marginBottom: 10, fontSize: 14 }}
                  placeholder='e.g. "Create a slideshow introducing fractions for Year 4"'
                  value={prompt}
                  onChange={e => setPrompt(e.target.value)}
                />
                <div style={{ marginBottom: 14 }}>
                ...
                <button
                  ...
                  style={s.submitBtn(generating || !prompt.trim())}
                >
                  {generating ? '⏳ Generating slideshow...' : '✨ Generate slideshow'}
                </button>
              </div>
            )}
            ...
            {tab === 'plan' && (
              <div>
                <p style={{ fontSize: 13, color: MUTED, marginBottom: 16, lineHeight: 1.6 }}>
                  Select a plan, then choose one or more lessons. Each lesson generates its own slideshow — downloaded separately and saved to My presentations.
                </p>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                    ...
                    <select value={planFilterSubject} onChange={e => { setPlanFilterSubject(e.target.value); setPlanDropdownOpen(true) }}
                      style={{ height: 32, fontSize: 12, borderRadius: 8, border: `0.5px solid ${planFilterSubject !== 'All' ? GREEN : BORDER}`, padding: "0 10px", background: planFilterSubject !== 'All' ? LIGHT_GREEN : BG, color: planFilterSubject !== 'All' ? '#085041' : TEXT, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", outline: "none" }}>
                      {planSubjects.map(sub => <option key={sub} value={sub}>{sub === 'All' ? 'All subjects' : sub}</option>)}
                    </select>
                    <select value={planFilterYear} onChange={e => { setPlanFilterYear(e.target.value); setPlanDropdownOpen(true) }}
                      style={{ height: 32, fontSize: 12, borderRadius: 8, border: `0.5px solid ${planFilterYear !== 'All' ? GREEN : BORDER}`, padding: "0 10px", background: planFilterYear !== 'All' ? LIGHT_GREEN : BG, color: planFilterYear !== 'All' ? '#085041' : TEXT, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", outline: "none" }}>
                      {planYears.map(y => <option key={y} value={y}>{y === 'All' ? 'All years' : y}</option>)}
                  ...
                  {selectedPlan && !planDropdownOpen ? (
                    <div style={{ padding: "10px 14px", borderRadius: 8, border: `0.5px solid ${GREEN}`, background: LIGHT_GREEN, display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <button onClick={() => { setPlanDropdownOpen(true); setSelectedPlan(null); setSelectedLessonNums([]) }}
                        style={{ fontSize: 11, color: MUTED, background: "none", border: `0.5px solid ${BORDER}`, borderRadius: 6, padding: "3px 8px", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                        Change
                      </button>
                    </div>
                    ...
                    <div style={{ border: `0.5px solid ${BORDER}`, borderRadius: 8, overflow: "hidden", maxHeight: 260, overflowY: "auto" }}>
                      {filteredPlans.length === 0 ? (
                        ...
                        <div key={plan.id}
                          onClick={() => { setSelectedPlanGroup(plan.group); setSelectedPlan(plan); setSelectedLessonNums([]); setPlanDropdownOpen(false) }}
                          style={{ padding: "10px 14px", borderBottom: `0.5px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 10, cursor: "pointer", background: BG }}
                          onMouseEnter={e => e.currentTarget.style.background = PAGE_BG}
                          onMouseLeave={e => e.currentTarget.style.background = BG}>
                          <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 20, background: PAGE_BG, color: MUTED, flexShrink: 0 }}>{plan.subject}</span>
                ...
                {/* Step 2 — Pick lesson(s) */}
                {selectedPlan && !planDropdownOpen && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: MUTED, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>
                      Step 2 — Select one or more lessons
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      {realLessons.map(lesson => {
                        const isSelected = selectedLessonNums.includes(lesson.lesson_number)
                        const tc = typeColors[lesson.type] || GREEN
                        const tb = typeBgs[lesson.type] || LIGHT_GREEN
                        return (
                          <div key={lesson.lesson_number}
                            ...
                            style={{ padding: "10px 14px", borderRadius: 8, border: `0.5px solid ${isSelected ? GREEN : BORDER}`, background: isSelected ? LIGHT_GREEN : BG, cursor: "pointer", display: "flex", alignItems: "flex-start", gap: 12 }}>
                            <div style={{ width: 20, height: 20, borderRadius: 4, border: `1.5px solid ${isSelected ? GREEN : BORDER}`, background: isSelected ? GREEN : BG, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#fff", flexShrink: 0, marginTop: 2 }}>
                              {isSelected ? '✓' : ''}
                            </div>
                            <div style={{ flex: 1 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 3, flexWrap: 'wrap' }}>
                                <span style={{ fontSize: 13, fontWeight: 500, color: isSelected ? "#085041" : TEXT }}>L{lesson.lesson_number} · {lesson.title}</span>
                                <span style={{ fontSize: 10, fontWeight: 600, color: tc, background: tb, padding: "1px 7px", borderRadius: 20 }}>{lesson.type}</span>
                              </div>
                              <div style={{ fontSize: 12, color: MUTED, lineHeight: 1.4 }}>
                ...
                {/* Generate button */}
                {selectedLessonNums.length > 0 && (
                  <button onClick={generateFromPlan} disabled={generating} style={s.submitBtn(generating)}>
                    {generating
                      ? `⏳ Generating slideshow ${generatedCount + 1} of ${selectedLessonNums.length}...`
                      : `✨ Generate ${selectedLessonNums.length} slideshow${selectedLessonNums.length !== 1 ? 's' : ''}`}
                  </button>
                )}
              </div>
            )}
            ...
            {tab === 'catalogue' && (
              <div>
                ...
                {catalogueLoading ? (
                  <div style={{ textAlign: 'center', padding: '2rem', color: MUTED, fontSize: 13 }}>Loading your presentations...</div>
                ) : (() => {
                  ...
                  if (filtered.length === 0) return (
                    <div style={{ textAlign: 'center', padding: '2.5rem', color: MUTED }}>
                      <div style={{ fontSize: 36, marginBottom: 10 }}>🎬</div>
                      <div style={{ fontSize: 14, fontWeight: 500, color: TEXT, marginBottom: 6 }}>
                        {catalogue.length === 0 ? 'No presentations yet' : 'No presentations match your search'}
                      </div>
                      <div style={{ fontSize: 13 }}>
                        {catalogue.length === 0 ? 'Generate a slideshow using Quick presentation or From a plan to see it here.' : 'Try a different search term.'}
                      </div>
                    </div>
                  )
                  ...
                  return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {filtered.map(pres => {
                        const isViewing = viewingPresentation?.id === pres.id
                        const isConfirmingDelete = confirmDeleteId === pres.id
                        const isRegenerating = regeneratingId === pres.id
                        return (
                          <div key={pres.id} style={{ border: `0.5px solid ${isViewing ? GREEN : BORDER}`, borderRadius: 10, background: BG, overflow: 'hidden' }}>
                            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 14px', cursor: 'pointer', flexWrap: 'wrap' }}
                              onClick={() => setViewingPresentation(isViewing ? null : pres)}>
                              ...
                              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                                <button
                                  onClick={async e => {
                                    e.stopPropagation()
                                    const newVal = !pres.is_favourite
                                    setCatalogue(prev => prev.map(p => p.id === pres.id ? { ...p, is_favourite: newVal } : p))
                                    const { error } = await supabase.from('presentations').update({ is_favourite: newVal }).eq('id', pres.id)
                                    if (error) {
                                      console.error('Favourite toggle error:', error.message)
                                      setCatalogue(prev => prev.map(p => p.id === pres.id ? { ...p, is_favourite: !newVal } : p))
                                    }
                                  }}
                                  title={pres.is_favourite ? 'Remove from favourites' : 'Add to favourites'}
                                  style={{ height: 26, width: 26, padding: 0, background: pres.is_favourite ? '#FEF3C7' : PAGE_BG, border: `0.5px solid ${pres.is_favourite ? '#F59E0B' : BORDER}`, borderRadius: 6, fontSize: 13, color: pres.is_favourite ? '#D97706' : MUTED, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                  {pres.is_favourite ? '⭐' : '☆'}
                                </button>
                                <button
                                  onClick={e => { e.stopPropagation(); setViewingPresentation(isViewing ? null : pres) }}
                                  style={{ height: 26, padding: '0 10px', background: isViewing ? LIGHT_GREEN : PAGE_BG, border: `0.5px solid ${isViewing ? GREEN : BORDER}`, borderRadius: 6, fontSize: 11, fontWeight: 500, color: isViewing ? '#085041' : TEXT, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                                  {isViewing ? 'Hide' : 'View'}
                                </button>
                                <PresentationDownloadButton presentation={pres} />
                                <button
                                  onClick={e => { e.stopPropagation(); handleRegenerate(pres) }}
                                  disabled={isRegenerating}
                                  title="Regenerate with AI"
                                  style={{ height: 26, padding: '0 10px', background: isRegenerating ? PAGE_BG : BG, border: `0.5px solid ${BORDER}`, borderRadius: 6, fontSize: 11, fontWeight: 500, color: isRegenerating ? MUTED : TEXT, cursor: isRegenerating ? 'wait' : 'pointer', fontFamily: "'DM Sans', sans-serif", whiteSpace: 'nowrap' }}>
                                  {isRegenerating ? '⏳' : '🔄 Regenerate'}
                                </button>
                                {isConfirmingDelete ? (
                                  <>
                                    <button
                                      onClick={async e => { e.stopPropagation(); await supabase.from('presentations').delete().eq('id', pres.id); setConfirmDeleteId(null); if (isViewing) setViewingPresentation(null); loadCatalogue() }}
                                      style={{ height: 26, padding: '0 10px', background: '#FCEBEB', border: '0.5px solid #A32D2D', borderRadius: 6, fontSize: 11, fontWeight: 600, color: '#A32D2D', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                                      Delete
                                    </button>
                                    <button
                                  ...
                                  <button
                                    onClick={e => { e.stopPropagation(); setConfirmDeleteId(pres.id) }}
                                    style={{ height: 26, padding: '0 8px', background: PAGE_BG, border: `0.5px solid ${BORDER}`, borderRadius: 6, fontSize: 11, color: MUTED, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                                    🗑️
                                  </button>
                                )}
                            ...
                            {/* Expanded view — slide list */}
                            {isViewing && (
                              <div style={{ borderTop: `0.5px solid ${BORDER}` }}>
                                {(pres.slides || []).map((sl, i) => (
                                  <div key={i} style={{ borderTop: i > 0 ? `0.5px solid ${BORDER}` : 'none', padding: '10px 14px', background: BG }}>
...
function UnitOverviewPage({ onNavigate, initialPlanId }) {
  ...
  useEffect(() => {
    ...
    async function loadPlans() {
      ...
      const { data, error } = await supabase
        .from('plans')
        .select('id, title, subject, year_group, lesson_count, book_title, book_author, created_at')
        .eq('user_id', user?.id)
        .order('created_at', { ascending: false })
      if (!error && data) {
        const groups = []
        data.forEach(plan => {
          const existing = groups.find(g => g.book.title === plan.book_title)
          const entry = { id: plan.id, title: plan.title, subject: plan.subject, lessons: plan.lesson_count, yearGroup: plan.year_group }
          if (existing) existing.plans.push(entry)
          else groups.push({ book: { title: plan.book_title, author: plan.book_author || '' }, plans: [entry] })
        })
  ...
  const tabBtn = (id, label, count) => (
    <button onClick={() => setTab(id)}
      style={{ border: 'none', borderBottom: tab === id ? `2px solid ${GREEN}` : '2px solid transparent', borderRadius: 0, background: 'transparent', fontWeight: tab === id ? 600 : 400, color: tab === id ? GREEN : MUTED, padding: '10px 16px', cursor: 'pointer', fontSize: 14, fontFamily: "'DM Sans', sans-serif" }}>
    ...
    <div style={{ ...s.page, maxWidth: "100%" }}>
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 1rem" }}>
        ...
        {!selectedPlanId ? (
          // ── Plan picker ──
          loadingPlans ? (
            <div style={{ textAlign: 'center', padding: '2rem', color: MUTED, fontSize: 13 }}>Loading your units...</div>
          ) : planGroups.length === 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: '2rem', color: MUTED, fontSize: 13 }}>Loading unit...</div>
        ) : (
          // ── Unit detail ──
          <div>
            <div onClick={backToPicker} style={{ fontSize: 12, color: GREEN, cursor: 'pointer', marginBottom: 12, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              ← All units
            </div>

            {/* Book card */}
            <div style={{ background: BG, border: `0.5px solid ${BORDER}`, borderRadius: 12, padding: '1rem 1.25rem', marginBottom: 16, display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{ width: 52, height: 64, background: LIGHT_GREEN, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 24 }}>📚</div>
              <div style={{ flex: '1 1 200px', minWidth: 0 }}>
                <div style={{ fontFamily: "'Lora', serif", fontSize: 17, fontWeight: 500, color: TEXT }}>{unit.plan.book_title}</div>
                <div style={{ fontSize: 12, color: MUTED, fontStyle: 'italic', marginBottom: 6 }}>{unit.plan.book_author}</div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: PAGE_BG, color: TEXT }}>{unit.plan.year_group}</span>
                  <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: PAGE_BG, color: TEXT }}>{unit.plan.subject}</span>
                  <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: PAGE_BG, color: TEXT }}>{unit.lessons.length} lessons</span>
                </div>
              </div>
            </div>
